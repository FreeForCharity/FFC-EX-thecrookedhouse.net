...
def main():
    ...
    # Create a version snapshot. Publishing is intentionally manual (last-mile
    # safety): a human clicks "Submit" then "Publish" in the GTM UI when ready
    # to push live. The script just gets everything into the workspace.
    if not args.dry_run:
        try:
            ...
            if v:
                ver_id = v.get("containerVersionId", "?")
                print(f"\nCreated container version {ver_id} "
                      f"(still in draft — publish manually in GTM UI when ready).")
            else:
                print("\nNo changes to snapshot (workspace already in sync).")
        except Exception as e:
            print(f"\nVersion snapshot skipped ({type(e).__name__}). "
                  f"Tags/triggers are in the workspace and can be reviewed "
                  f"and published manually in the GTM UI.")
